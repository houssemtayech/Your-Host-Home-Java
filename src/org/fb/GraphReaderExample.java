...
public class GraphReaderExample {
   ...
   */
    public static String testmail;  // declarer champ mail static pour le recuperer dans une autre interface 
  ...
  public static void main(String[] args) {
    new GraphReaderExample("CAACEdEose0cBACTBNF48vY6anjD5ROdMa03EE2tZA7dszZAPTi3nTFYZAeq0nV5TZCZBPyUt1b1TNcl18LZAMkFAqwPyUcTQ2HByhFBqSXzT3GAlBYsUavPfTdZAAmm0agCC0ZChoPrZCKOPG157ubsiYB8GJ9YM6jwdhJYTA6QbsrUMdjdQlJms6DcadUwIerUcuHsZCZCcfkeBgZDZD").runEverything();
  }
...
//    rawJsonResponse();
   ...
   public void fetchObject() {
    ...
    User user = getFacebookClient().fetchObject("me", User.class);
    Page page = getFacebookClient().fetchObject("cocacola", Page.class);
    ...
    testmail = user.getEmail();
    //out.println("Page likes: " + page.getLikes());
  }

   public void fetchObjectsAsJsonObject() {
    ...
    // Make the API call
    JsonObject results = getFacebookClient().fetchObjects(ids, JsonObject.class);
   ...
   public void fetchObjects() {
    ...
    FetchObjectsResults fetchObjectsResults =
        getFacebookClient().fetchObjects(Arrays.asList("me", "cocacola"), FetchObjectsResults.class);

    out.println("User name: " + fetchObjectsResults.me.getName());
    out.println("Page likes: " + fetchObjectsResults.page.getLikes());
  }

   public void fetchDifferentDataTypesAsJsonObject() {
    out.println("* Fetching different types of data as JsonObject *");

    JsonObject btaylor = getFacebookClient().fetchObject("btaylor", JsonObject.class);
    out.println(btaylor.getString("name"));

    JsonObject photosConnection = getFacebookClient().fetchObject("me/photos", JsonObject.class);
    ...
    List<JsonObject> queryResults = getFacebookClient().executeQuery(query, JsonObject.class);
  ...
  public static class FetchObjectsResults {
   ...
   public void fetchConnections() {
    ...
    Connection<User> myFriends = getFacebookClient().fetchConnection("me/friends", User.class);
    Connection<Post> myFeed = getFacebookClient().fetchConnection("me/feed", Post.class);
   ...
   public void query() {
    ...
    List<FqlUser> users =
        getFacebookClient().executeQuery("SELECT uid, name FROM user WHERE uid=220439 or uid=7901103", FqlUser.class);
   ...
   public void multiquery() {
    ...
    MultiqueryResults multiqueryResults = getFacebookClient().executeMultiquery(queries, MultiqueryResults.class);
  ...
  public static class MultiqueryResults {
    ...
    Connection<Post> publicSearch =
        getFacebookClient().fetchConnection("search", Post.class, Parameter.with("q", "watermelon"),
          Parameter.with("type", "post"));

    Connection<User> targetedSearch =
        getFacebookClient().fetchConnection("me/home", User.class, Parameter.with("q", "Mark"),
   ...
   public void metadata() {
    ...
    User userWithMetadata = getFacebookClient().fetchObject("me", User.class, Parameter.with("metadata", 1));
   ...
   public void paging() {
    ...
    Connection<User> myFriends = getFacebookClient().fetchConnection("me/friends", User.class);
    Connection<Post> myFeed = getFacebookClient().fetchConnection("me/feed", Post.class, Parameter.with("limit", 100));
   ...
   public void selection() {
    ...
    User user = getFacebookClient().fetchObject("me", User.class, Parameter.with("fields", "id,name"));
  ...
  public void parameters() {
    ...
    Connection<Post> filteredFeed =
        getFacebookClient().fetchConnection("me/feed", Post.class, Parameter.with("limit", 3),
          Parameter.with("until", "yesterday"), Parameter.with("since", oneWeekAgo));

    out.println("Filtered feed count: " + filteredFeed.getData().size());
  }

   public void rawJsonResponse() {
    out.println("* Raw JSON *");
    out.println("User object JSON: " + getFacebookClient().fetchObject("me", String.class));
  }
}